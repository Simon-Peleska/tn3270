...
test("the panel opens on its command line and Tab walks onto the body, skipping headings", () => {
  ...
  assert.equal(
    page.onCommand,
    true,
    "up from the first field is the command line again",
  );
});
...
test("copy and paste work on the panel, as they do on the screen", () => {
  ...
  assert.equal(
    page.copy(),
    "value 2",
    "with the cursor in the body, the field is what is copied",
  );
});
...
test("nothing any panel writes is wider than an 80-column screen", () => {
  ...
  for (const page of pages) {
    for (const note of page.notes()) {
      assert.ok(
        note.length <= 79,
        `${page.id} note is ${note.length} long: ${note}`,
      );
    }
    const legend = page.keys().join("  ");
    assert.ok(
      legend.length <= 79,
      `${page.id} legend is ${legend.length} long: ${legend}`,
    );
    ...
    for (const line of page.lines()) {
      ...
      assert.ok(
        ...
        `${page.id} label "${text}" is wider than its ${labelWidth}`,
      );
      const right =
        TEXT_LEFT + labelWidth + value.length + (line.field === true ? 2 : 0);
      assert.ok(
        right <= 80,
        `${page.id} value "${value}" ends at column ${right}`,
      );
    }
  }
});
...
test("the help panel lists the keys, the commands and the shortcuts", () => {
  const help = new HelpPage(fixture().deps);
  const text = help
    .lines()
