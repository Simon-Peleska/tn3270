...
async function waitForAnswer(url, cookie) {
  ...
  const headers =
    cookie === undefined ? {} : { cookie: `x3270-security=${cookie}` };
  let answered = false;
  await waitUntil(() => {
    if (!answered) {
      fetch(url, { headers })
...
test("GET /api/sessions/<id>/3270/rest/json/<action> answers one action, s3270-httpd style", async (t) => {
  ...
  const failed = await fetch(
    `${base}/api/sessions/${created.id}/3270/rest/json/BogusAction()`,
  );
  ...
  const missingSession = await fetch(
    `${base}/api/sessions/${"0".repeat(8)}-0000-0000-0000-000000000000/3270/rest/json/Query(CodePage)`,
  );
...
test("the text and stext flavours are forwarded too, not just json", async (t) => {
  ...
  assert.ok(
    lines[0]?.startsWith("L U U N N"),
    `status line first, got ${JSON.stringify(lines[0])}`,
  );
});
...
test("an awkward argument survives the proxy and reaches the screen", async (t) => {
  ...
  const rest = async (call) => {
    const response = await fetch(
      `${sessionBase}/3270/rest/json/${encodeURIComponent(call)}`,
    );
...
test("the emulator's own REST port is shut to anyone without the session cookie", async (t) => {
  ...
  assert.equal(
    uninvited.status,
    403,
    "a local process that guesses the port must still be refused",
  );
  assert.match(await uninvited.text(), /cookie/i);

  const wrong = await fetch(url, {
    headers: { cookie: "x3270-security=not-the-cookie" },
  });
...
test("a REST call runs even for a viewer sharing would refuse", async (t) => {
  ...
  assert.ok(
    observer.messages.some(
      (m) => (m.type === "error" ? m.code : "") === "E3006",
    ),
    "the observer must be refused over the WebSocket",
  );

  const url = `http://127.0.0.1:${endpoint.port}/3270/rest/json/Query(CodePage)`;
  await waitForAnswer(url, endpoint.cookie);
  const answer = await fetch(url, {
    headers: { cookie: `x3270-security=${endpoint.cookie}` },
  });
...
test("comparison: what we forward is byte for byte what a real s3270 -httpd answers", async (t) => {
  ...
  const oracle = spawn(
    s3270,
    ["-model", "2", "-httpd", `127.0.0.1:${oraclePort}`],
    { stdio: "pipe" },
  );
  t.after(() => oracle.kill());
  await waitForAnswer(
    `http://127.0.0.1:${oraclePort}/3270/rest/json/Query(CodePage)`,
  );
  ...
  for (const flavour of ["json", "text", "stext"]) {
    for (const call of [
      ...
      const [ourBody, theirBody] = await Promise.all([
        ours.text(),
        theirs.text(),
      ]);
      assert.equal(ours.status, theirs.status, `status for ${path}`);
      assert.equal(
        ours.headers.get("content-type"),
        theirs.headers.get("content-type"),
