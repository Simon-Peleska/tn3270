...
test("a session line carries the session id, and a viewer line its address too", (t) => {
  ...
  const log = logger("session", {
    session: "a5dd6b0e-0000-4000-8000-000000000001",
  });

  log.info("connecting", { host: "127.0.0.1:4001" });
  log
    .with({ viewer: "9894d691", ip: "10.0.0.7" })
    .info("viewer attached", { role: "controller" });

  const lines = readFileSync(path, "utf8").trimEnd().split("\n");
  assert.equal(lines.length, 2);
  assert.match(
    lines[0] ?? "",
    /INFO {2}session connecting session=a5dd6b0e-0000-4000-8000-000000000001 host=127\.0\.0\.1:4001/,
  );
  assert.match(
    lines[1] ?? "",
    /session=a5dd6b0e-0000-4000-8000-000000000001 viewer=9894d691 ip=10\.0\.0\.7 role=controller/,
  );
});
...
test("a field nobody filled in is left off the line", (t) => {
  ...
  logger("session", { session: "abc", user: "" }).info("viewer attached", {
    ip: "10.0.0.7",
    role: "",
  });
...
test("the file rolls over once it passes its limit, keeping the lines before it", (t) => {
  ...
  for (let i = 0; i < 30; i++)
    log.info("filler", { i, padding: "x".repeat(100) });
  ...
  assert.ok(
    rolled.length >= maxBytes,
    `the rolled file should be full, was ${rolled.length}`,
  );
  assert.ok(
    current.length < maxBytes,
    `the current file should have restarted, was ${current.length}`,
  );
});
...
test("an error keeps its stack, and its context, in the file", (t) => {
  ...
  assert.match(
    text,
    /ERROR session \[E0000\] something broke session=errs ip=10\.0\.0\.7/,
  );
