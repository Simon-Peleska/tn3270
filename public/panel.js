 ...
 * no number of its own: you get back to it with F4, or by naming it.
 *
 * @type {readonly { id: string, option: string, name: string, blurb: string }[]}
 */
export const PANELS = Object.freeze([
  { id: 'menu', option: '', name: 'Menu', blurb: 'The panel every other one is reached from' },
  { id: 'settings', option: '0', name: 'Settings', blurb: 'Colours, font, screen size and sharing' },
  { id: 'macros', option: '1', name: 'Macros', blurb: 'Record, play back and trade keystroke macros' },
  { id: 'recorder', option: '2', name: 'Recorder', blurb: 'Capture screens and keys as a script' },
  { id: 'keymap', option: '3', name: 'Keys', blurb: 'What each key and key combination does' },
  { id: 'help', option: 'H', name: 'Help', blurb: 'The keys and commands panels answer to' },
]);
...
export function panelIdForOption(option) {
  ...
  const found = PANELS.find((panel) => panel.option !== '' && panel.option === wanted);
 ...
 */
export function panelColors(theme) {
...
export const ROW_TITLE = 3;
export const ROW_COMMAND = 4;
export const ROW_MESSAGE = 5;
export const BODY_TOP = 7;

/** @returns {{ text: string, spans: { id: string, start: number, end: number }[] }} 1-based columns */
export function actionBar() {
  ...
  for (const panel of PANELS) {
    ...
    text += panel.name;
    spans.push({ id: panel.id, start, end: text.length });
    text += '  ';
  }
  return { text, spans };
}
...
/**
 * The body row a screen row lands on, so a click can put the cursor there.
 *
 * @param {number} row 1-based terminal row
 * @returns {number} index into the drawn body, -1 for the command line, -2 elsewhere
 */
export function bodyIndexAt(row) {
  if (row === ROW_COMMAND) return -1;
  return row >= BODY_TOP ? row - BODY_TOP : -2;
}
...
export function dotted(label, width) {
  ...
  return (`${label} ${'. '.repeat(Math.ceil((width - label.length) / 2))}`).slice(0, width);
}
 ...
 * @returns {void}
 */
export function drawPanel(view) {
  const { cols, rows } = view.geometry();
  const { background, foreground, dim, field, chosen, warn } = panelColors(view.theme);
  ...
  const bar = actionBar();
  place(1, 1, paint(dim, background), bar.text);
  for (const span of bar.spans) {
    if (span.id !== view.panel) continue;
    const name = PANELS.find((panel) => panel.id === span.id)?.name ?? '';
    place(1, span.start, paint(background, foreground, true), name);
  }
  place(2, 1, paint(dim, background), '─'.repeat(cols));

  const title = view.title.slice(0, cols - 4);
  place(ROW_TITLE, Math.max(1, Math.floor((cols - title.length) / 2) + 1), paint(foreground, background, true), title);
  if (view.message !== undefined && view.message !== '') {
    const text = view.message.slice(0, Math.max(0, cols - 22));
    place(ROW_TITLE, Math.max(1, cols - text.length), paint(warn, background, true), text);
  }
  ...
  place(ROW_COMMAND, commandLeft, paint(foreground, view.onCommand ? chosen : field), view.command.slice(-commandWidth).padEnd(commandWidth));
  const more = `${first > 0 ? '-' : ' '}${first + view.body.length < total ? '+' : ' '}`;
  if (more !== '  ') place(ROW_COMMAND, cols - 8, paint(dim, background), `More: ${more}`);
  ...
  view.body.forEach((line, index) => {
    ...
    const style = paint(line.selected === true ? foreground : dim, background, line.selected === true);
    if (line.option !== undefined) place(bodyRow, OPTION_LEFT, paint(foreground, background, true), line.option.padStart(2));
    const text = line.text ?? '';
    if (text !== '') place(bodyRow, TEXT_LEFT, style, line.dots === true ? dotted(text, labelWidth) : text.padEnd(labelWidth));
    const value = line.value ?? '';
    if (value === '') return;
    const valueLeft = TEXT_LEFT + labelWidth + 1;
    if (line.field === true) {
      place(bodyRow, valueLeft, paint(foreground, line.selected === true ? chosen : field), ` ${value} `);
    } else {
      place(bodyRow, valueLeft, paint(line.selected === true ? foreground : dim, background), value);
    }
  });
  ...
  const onLine = view.body.find((line) => line.selected === true);
  const cursorRow = view.onCommand ? ROW_COMMAND : BODY_TOP + view.body.findIndex((line) => line.selected === true);
  const cursorCol = view.onCommand
    ? Math.min(cols, commandLeft + Math.min(view.command.length, commandWidth - 1))
    : TEXT_LEFT + labelWidth + 2 + (onLine?.cursor ?? 0);
  out.push(at(Math.max(1, Math.min(rows, cursorRow)), Math.max(1, Math.min(cols, cursorCol))), `${ESC}[?25h`);
 ...
 */
export function panelAction(event) {
  if (event.ctrlKey || event.metaKey) return null;
  switch (event.key) {
    case 'F1': return 'help';
    case 'F3': case 'Escape': return 'end';
    case 'F4': return 'return';
    case 'F7': case 'PageUp': return 'backward';
    case 'F8': case 'PageDown': return 'forward';
    case 'F12': return 'cancel';
...
export function parseCommand(input) {
  ...
  if (text === 'RET' || text === 'RETURN' || text === 'MENU') return { kind: 'return' };
...
export class Panel {
  ...
  move(step) {
    ...
    const current = this.onCommand ? 0 : Math.max(0, stops.indexOf(this.selected));
  ...
  scrollBy(step) {
    ...
    const first = this.stops().find((index) => index >= this.scroll && index < this.scroll + height);
    if (first !== undefined) {
  ...
  submit() {
    ...
    if (parsed.kind === 'select') {
      if (!this.pick(parsed.option)) this.say(`No line is numbered ${parsed.option}`);
      return;
    }
    // A word this panel knows, a panel named in the action bar, or nothing.
    if (this.word(parsed.word)) return;
    const named = PANELS.find((panel) => panel.name.toUpperCase() === parsed.word);
    if (named !== undefined) this.deps.go(named.id);
    else if (!this.pick(parsed.word)) this.say(`${typed.trim()} is not a command here`);
  }
  ...
  clicked(row, col) {
    const bar = actionBar();
    if (row === 1) {
      const hit = bar.spans.find((span) => col >= span.start && col <= span.end);
      if (hit !== undefined) this.deps.go(hit.id);
      return;
    }
    const index = bodyIndexAt(row);
    if (index === -1) {
      this.onCommand = true;
      this.draw();
      return;
  ...
  draw() {
    ...
    this.scroll = Math.max(0, Math.min(this.scroll, Math.max(0, lines.length - height)));

    const body = lines.slice(this.scroll, this.scroll + height).map((line, index) => ({
      ...line,
